...
def process_GR(filename, window_size = 7, omit=['blank'], control_condition = '2u:C', normalise=True, starting_row = 6):
    ...
    #discard any rows of the array where factors corresponds to a member of the omit list
    ...
    exponents,clean_factors = [],[]

    #process all samples (rows)
    for row in range(data.shape[0]):
        if factors[row] not in omit:
            ...
            clean_factors.append(factors[row])
    ...
    return pd.DataFrame({'Plasmid':clean_factors,'Rel_GR':exponents})
